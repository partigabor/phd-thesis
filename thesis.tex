...
% \babelfont{rm}[Language=Default]{FreeSerif} %, Ligatures={Common}, Scale=1.0
...
\newglossary[a]{primary}{b}{c}{Dictionaries}
...
% \AtBeginRefsection{\GenRefcontextData{sorting=ynt}} %error, need biblatex 3.18
% \AtEveryCite{\localrefcontext[sorting=ynt]} %error, need biblatex 3.18
...
\cleardoublepage % Works locally, but not on Overleaf!
\printglossary 
\printglossary[type=acronym]
\printglossary[type=primary, title={Primary Sources}, style=index]
\printglossary[type=symbols, title={Symbols and Notation}]
\printunsrtglossary[type=symbols, title={Symbols and Notation}]
...
\include{contents/6-names}
\include{contents/7-language}
\include{contents/9-conclusion}
