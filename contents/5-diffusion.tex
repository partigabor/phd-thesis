...
What we can know about the diffusion of spices beyond the botanical evidence, is in their names. In most cases, the spice names spread with the materials, and have left a trace. The following section will focus on this phenomenon.
...
The fact that English has many loanwords in the spice domain is not surprising if we understand that all of these aromatic products are exotic, they are not from anywhere near England, or the Saxon homeland. As for Arabic, we know from history that all materials from Asia passed through the Arabian Peninsula, and the names of many spices with origins in West Asia predate the Arabic expansion of the \nth{7} century and hence loanwords from other Semitic languages. Loanwords in Chinese in the spice domain are much fewer in number, most of the historic words being Silk Road terms.
...
\Cref{fig:diffusion_en} shows the diffusion of spice names, when viewed from the words' etymological stages into English. Words that were coined in English (i.e. not loanwords), are not present.  
